import { getCachedRows, parseBool, nullStr } from '@/lib/sheets'
...
export async function getTemplates(): Promise<BlockTemplate[]> {
  const [templates, btds, drillRows] = await Promise.all([
    getCachedRows('block_templates'),
    getCachedRows('block_template_drills'),
    getCachedRows('drills'),
  ])
  ...
  for (const btd of btds) {
    ...
    drillsByTemplate[btd.template_id].push({
      id: btd.id, template_id: btd.template_id, drill_id: btd.drill_id,
      sort_order: Number(btd.sort_order), drill,
    })
  }

  for (const id of Object.keys(drillsByTemplate)) {
    drillsByTemplate[id].sort((a: TemplateDrill, b: TemplateDrill) => a.sort_order - b.sort_order)
  }
